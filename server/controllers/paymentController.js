...
controllerMethods.payWithCard = async (req, res) => {
  try {
    ...
    let payment_result = await creditCardService.makePayment(
      credit_card,
      amount
    );
    ...
    if (!payment_result.success)
      ...
      const id = uuid();
      let result = await creditCardService.storeCreditCardPayment(
        id,
        billed_amount,
        credit_card,
        billed_amount,
...
controllerMethods.payBothWays = async (req, res) => {
  try {
    ...
    if (use_credit) {
      if (isRegisteredUser)
        credits = await creditService.getCreditByUser(user_id);
