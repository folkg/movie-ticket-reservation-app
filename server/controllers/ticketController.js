...
const router = require("../v1/routes/userRoutes");
...
controllerMethods.getAllTickets = async (req, res) => {
    try {
        let results = await ticketService.getAllTickets();
        if(results){
            res.json({status: true, data: results})
        } else {
            res.json({status: false, message: 'no tickets found'});
        }
    } catch (e) {
...
controllerMethods.cancelTicketById = async(req, res) => {
    try {
        ...
        if(!canCancel(show_time)) res.json({success: false, message: 'Showtime is in less than 72 hrs. Cannot Cancel.'});
        else {
            let credit = cost; 
            // Apply admin fee if the user is not registered. 
            if(!isRegisteredUser) credit = cost * (1 - constants.ADMIN_FEE);
            let results = await ticketService.cancelTicketById(ticket_id, seat_id, credit);
