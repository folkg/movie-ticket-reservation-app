...
serviceMethods.pay1 = ( req ) => {
  return new Promise( async (resolve, reject) => {
    try {
      ...
      if(billed_amount > 0) await serviceMethods.storeCreditCardPayment( payment_id, billed_amount, credit_card);
...
serviceMethods.pay2 = ( req ) => {
  return new Promise( async (resolve, reject) => {
    try{
      ...
      const { payment_id } = payment;
      if(billed_amount > 0) await serviceMethods.storeCreditCardPayment( payment_id, billed_amount, credit_card);
      if (credit.length > 0) await refundService.updateCredit(payment_id, credit);
      return resolve(payment);
    } catch(e) {
      reject(e);
    }
